...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec3 evaPurple = vec3(0.706, 0.553, 0.859);

    float trailWidth = max(max(iCurrentCursor.z, iCurrentCursor.w) * 0.72, 10.0);
    float trail = 1.0 - smoothstep(trailWidth * 0.15, trailWidth, lineDistance(fragCoord, previousCenter, currentCenter));
    trail *= fade * moved * 0.55;

    vec2 halfCursor = max(iCurrentCursor.zw * 0.5, vec2(5.0));
    float glowDistance = roundedBox(fragCoord, currentCenter, halfCursor + vec2(6.0), 6.0);
    float glow = (1.0 - smoothstep(0.0, 18.0, glowDistance)) * 0.25;

    vec3 color = terminal.rgb;
    color = mix(color, evaPurple, trail);
